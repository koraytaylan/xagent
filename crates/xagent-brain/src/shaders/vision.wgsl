// Vision: 48-ray march + proprioception/interoception/touch fill.
// 1 workgroup per agent, 48 threads per workgroup (1 per ray).
// Thread 0 also fills non-visual sensory data after workgroupBarrier.

@group(0) @binding(0) var<storage, read> agent_phys: array<f32>;
@group(0) @binding(1) var<storage, read> heightmap: array<f32>;
@group(0) @binding(2) var<storage, read> biome_grid: array<u32>;
@group(0) @binding(3) var<storage, read> food_state: array<f32>;
@group(0) @binding(4) var<storage, read> food_flags: array<u32>;
@group(0) @binding(5) var<storage, read> food_grid: array<u32>;
@group(0) @binding(6) var<storage, read> agent_grid: array<u32>;
@group(0) @binding(7) var<storage, read_write> sensory: array<f32>;
@group(0) @binding(8) var<uniform> wconfig: array<vec4<f32>, 6>;

fn wc(idx: u32) -> f32 {
    return wconfig[idx / 4u][idx % 4u];
}

fn biome_at(x: f32, z: f32) -> u32 {
    let half = wc(10u);
    let inv = wc(11u);
    let col = min(u32((x + half) * inv), 255u);
    let row = min(u32((z + half) * inv), 255u);
    return biome_grid[row * 256u + col];
}

@compute @workgroup_size(48)
fn main(
    @builtin(global_invocation_id) gid: vec3<u32>,
    @builtin(local_invocation_id) lid: vec3<u32>,
    @builtin(workgroup_id) wgid: vec3<u32>,
) {
    let agent = wgid.x;
    let agent_count = u32(wc(13u));
    if agent >= agent_count { return; }

    let ray_idx = lid.x;
    let b = agent * PHYS_STRIDE;
    let s_base = agent * SENSORY_STRIDE;

    // Skip dead agents — zero out this ray's contribution and bail
    let alive = agent_phys[b + P_ALIVE];
    if alive < 0.5 {
        let ci = s_base + ray_idx * 4u;
        sensory[ci]      = 0.0;
        sensory[ci + 1u] = 0.0;
        sensory[ci + 2u] = 0.0;
        sensory[ci + 3u] = 0.0;
        sensory[s_base + VISION_COLOR_COUNT + ray_idx] = 1.0;
        if ray_idx == 0u {
            let nv_base = s_base + VISION_COLOR_COUNT + VISION_DEPTH_COUNT;
            for (var i: u32 = 0u; i < 27u; i++) {
                sensory[nv_base + i] = 0.0;
            }
        }
        return;
    }

    // Read agent state
    let pos = vec3<f32>(
        agent_phys[b + P_POS_X],
        agent_phys[b + P_POS_Y],
        agent_phys[b + P_POS_Z],
    );
    let facing = vec3<f32>(
        agent_phys[b + P_FACING_X],
        agent_phys[b + P_FACING_Y],
        agent_phys[b + P_FACING_Z],
    );

    // ── Ray direction computation ──────────────────────────────────────
    let col = ray_idx % VISION_W;
    let row = ray_idx / VISION_W;
    let u = (f32(col) / f32(VISION_W - 1u)) * 2.0 - 1.0;
    let v = (f32(row) / f32(VISION_H - 1u)) * 2.0 - 1.0;
    let tan_hf = tan(VISION_FOV_HALF);
    let right = vec3<f32>(facing.z, 0.0, -facing.x);
    let ray_dir = normalize(facing + right * u * tan_hf + vec3<f32>(0.0, -v * tan_hf, 0.0));

    // ── Ray march ──────────────────────────────────────────────────────
    var hit_color = vec4<f32>(0.53, 0.81, 0.92, 1.0); // sky default
    var hit_depth = VISION_MAX_DIST;
    var hit = false;

    for (var step: u32 = 0u; step < VISION_NUM_STEPS; step++) {
        if hit { break; }

        let t = f32(step + 1u) * VISION_STEP_SIZE;
        let ray_pos = pos + ray_dir * t;

        // ── Check food grid ────────────────────────────────────────
        let food_cx = cell_coord(ray_pos.x) + GRID_OFFSET;
        let food_cz = cell_coord(ray_pos.z) + GRID_OFFSET;

        for (var di: i32 = -1; di <= 1; di++) {
            if hit { break; }
            for (var dj: i32 = -1; dj <= 1; dj++) {
                if hit { break; }
                let ncx = food_cx + di;
                let ncz = food_cz + dj;
                if ncx < 0 || ncz < 0 { continue; }
                let uncx = u32(ncx);
                let uncz = u32(ncz);
                if uncx >= GRID_WIDTH || uncz >= GRID_WIDTH { continue; }

                let cell_idx = uncx * GRID_WIDTH + uncz;
                let cell_base = cell_idx * FOOD_GRID_CELL_STRIDE;
                let count = min(food_grid[cell_base], FOOD_GRID_MAX_PER_CELL);

                for (var s: u32 = 0u; s < count; s++) {
                    let fidx = food_grid[cell_base + 1u + s];
                    if food_flags[fidx] != 0u { continue; } // consumed

                    let fbase = fidx * FOOD_STATE_STRIDE;
                    let fx = food_state[fbase + F_POS_X];
                    let fy = food_state[fbase + F_POS_Y];
                    let fz = food_state[fbase + F_POS_Z];

                    let dx = ray_pos.x - fx;
                    let dy = ray_pos.y - fy;
                    let dz = ray_pos.z - fz;
                    let dist_sq = dx * dx + dy * dy + dz * dz;

                    if dist_sq < FOOD_RAY_RADIUS_SQ {
                        hit_color = vec4<f32>(0.7, 0.95, 0.2, 1.0);
                        hit_depth = t;
                        hit = true;
                        break;
                    }
                }
            }
        }
        if hit { break; }

        // ── Check agent grid ───────────────────────────────────────
        let ag_cx = cell_coord(ray_pos.x) + GRID_OFFSET;
        let ag_cz = cell_coord(ray_pos.z) + GRID_OFFSET;

        for (var di: i32 = -1; di <= 1; di++) {
            if hit { break; }
            for (var dj: i32 = -1; dj <= 1; dj++) {
                if hit { break; }
                let ncx = ag_cx + di;
                let ncz = ag_cz + dj;
                if ncx < 0 || ncz < 0 { continue; }
                let uncx = u32(ncx);
                let uncz = u32(ncz);
                if uncx >= GRID_WIDTH || uncz >= GRID_WIDTH { continue; }

                let cell_idx = uncx * GRID_WIDTH + uncz;
                let cell_base = cell_idx * AGENT_GRID_CELL_STRIDE;
                let count = min(agent_grid[cell_base], AGENT_GRID_MAX_PER_CELL);

                for (var s: u32 = 0u; s < count; s++) {
                    let other = agent_grid[cell_base + 1u + s];
                    if other == agent { continue; }

                    let ob = other * PHYS_STRIDE;
                    if agent_phys[ob + P_ALIVE] < 0.5 { continue; }

                    let ox = agent_phys[ob + P_POS_X];
                    let oy = agent_phys[ob + P_POS_Y];
                    let oz = agent_phys[ob + P_POS_Z];

                    let dx = ray_pos.x - ox;
                    let dy = ray_pos.y - oy;
                    let dz = ray_pos.z - oz;
                    let dist_sq = dx * dx + dy * dy + dz * dz;

                    if dist_sq < AGENT_RAY_RADIUS_SQ {
                        hit_color = vec4<f32>(0.9, 0.2, 0.6, 1.0);
                        hit_depth = t;
                        hit = true;
                        break;
                    }
                }
            }
        }
        if hit { break; }

        // ── Check terrain ──────────────────────────────────────────
        let gx = clamp((ray_pos.x + TERRAIN_HALF) * TERRAIN_INV_STEP, 0.0, TERRAIN_MAX_COORD);
        let gz = clamp((ray_pos.z + TERRAIN_HALF) * TERRAIN_INV_STEP, 0.0, TERRAIN_MAX_COORD);
        let ix = min(u32(gx), TERRAIN_MAX_IDX);
        let iz = min(u32(gz), TERRAIN_MAX_IDX);
        let fx = gx - f32(ix);
        let fz = gz - f32(iz);
        let h00 = heightmap[iz * TERRAIN_VPS + ix];
        let h10 = heightmap[iz * TERRAIN_VPS + ix + 1u];
        let h01 = heightmap[(iz + 1u) * TERRAIN_VPS + ix];
        let h11 = heightmap[(iz + 1u) * TERRAIN_VPS + ix + 1u];
        let ground_h = mix(mix(h00, h10, fx), mix(h01, h11, fx), fz);

        if ray_pos.y <= ground_h {
            let biome_type = biome_at(ray_pos.x, ray_pos.z);
            if biome_type == 0u {
                // FoodRich
                hit_color = vec4<f32>(0.15, 0.5, 0.1, 1.0);
            } else if biome_type == 1u {
                // Barren
                hit_color = vec4<f32>(0.5, 0.4, 0.2, 1.0);
            } else {
                // Danger
                hit_color = vec4<f32>(0.6, 0.2, 0.1, 1.0);
            }
            hit_depth = t;
            hit = true;
            break;
        }

        // Early-out for upward rays clearly above terrain
        if ray_dir.y > 0.3 && ray_pos.y > ground_h + 5.0 {
            break; // sky
        }
    }

    // ── Write vision results ───────────────────────────────────────────
    let ci = s_base + ray_idx * 4u;
    sensory[ci]      = hit_color.x;
    sensory[ci + 1u] = hit_color.y;
    sensory[ci + 2u] = hit_color.z;
    sensory[ci + 3u] = hit_color.w;
    sensory[s_base + VISION_COLOR_COUNT + ray_idx] = hit_depth / VISION_MAX_DIST;

    // ── Non-visual data (thread 0 only) ────────────────────────────────
    workgroupBarrier();

    if ray_idx != 0u { return; }

    let nv_base = s_base + VISION_COLOR_COUNT + VISION_DEPTH_COUNT;
    var off = nv_base;

    // Velocity (3 floats)
    sensory[off]      = agent_phys[b + P_VEL_X];
    sensory[off + 1u] = agent_phys[b + P_VEL_Y];
    sensory[off + 2u] = agent_phys[b + P_VEL_Z];
    off += 3u;

    // Facing (3 floats)
    sensory[off]      = agent_phys[b + P_FACING_X];
    sensory[off + 1u] = agent_phys[b + P_FACING_Y];
    sensory[off + 2u] = agent_phys[b + P_FACING_Z];
    off += 3u;

    // Angular velocity (1 float)
    sensory[off] = agent_phys[b + P_ANGULAR_VEL];
    off += 1u;

    // Energy signal (1 float)
    let energy = agent_phys[b + P_ENERGY];
    let max_energy = agent_phys[b + P_MAX_ENERGY];
    sensory[off] = energy / max(max_energy, 1e-6);
    off += 1u;

    // Integrity signal (1 float)
    let integrity = agent_phys[b + P_INTEGRITY];
    let max_integrity = agent_phys[b + P_MAX_INTEGRITY];
    sensory[off] = integrity / max(max_integrity, 1e-6);
    off += 1u;

    // Energy delta (1 float)
    let prev_energy = agent_phys[b + P_PREV_ENERGY];
    sensory[off] = energy - prev_energy;
    off += 1u;

    // Integrity delta (1 float)
    let prev_integrity = agent_phys[b + P_PREV_INTEGRITY];
    sensory[off] = integrity - prev_integrity;
    off += 1u;

    // ── Touch contacts (4 slots × 4 features = 16 floats) ─────────────
    var touch_count: u32 = 0u;
    let touch_base = off;

    // Zero all touch slots first
    for (var i: u32 = 0u; i < MAX_TOUCH_CONTACTS * 4u; i++) {
        sensory[touch_base + i] = 0.0;
    }

    // Query food_grid for nearby food within TOUCH_FOOD_RANGE
    let self_cx = cell_coord(pos.x) + GRID_OFFSET;
    let self_cz = cell_coord(pos.z) + GRID_OFFSET;

    for (var di: i32 = -1; di <= 1; di++) {
        for (var dj: i32 = -1; dj <= 1; dj++) {
            if touch_count >= MAX_TOUCH_CONTACTS { break; }
            let ncx = self_cx + di;
            let ncz = self_cz + dj;
            if ncx < 0 || ncz < 0 { continue; }
            let uncx = u32(ncx);
            let uncz = u32(ncz);
            if uncx >= GRID_WIDTH || uncz >= GRID_WIDTH { continue; }

            let cell_idx = uncx * GRID_WIDTH + uncz;
            let cell_base = cell_idx * FOOD_GRID_CELL_STRIDE;
            let count = min(food_grid[cell_base], FOOD_GRID_MAX_PER_CELL);

            for (var s: u32 = 0u; s < count; s++) {
                if touch_count >= MAX_TOUCH_CONTACTS { break; }
                let fidx = food_grid[cell_base + 1u + s];
                if food_flags[fidx] != 0u { continue; }

                let fbase = fidx * FOOD_STATE_STRIDE;
                let fdx = food_state[fbase + F_POS_X] - pos.x;
                let fdz = food_state[fbase + F_POS_Z] - pos.z;
                let dist = sqrt(fdx * fdx + fdz * fdz);

                if dist < TOUCH_FOOD_RANGE {
                    let slot = touch_base + touch_count * 4u;
                    let inv_dist = 1.0 / max(dist, 1e-6);
                    sensory[slot]      = fdx * inv_dist;       // dir_x
                    sensory[slot + 1u] = fdz * inv_dist;       // dir_z
                    sensory[slot + 2u] = 1.0 - dist / TOUCH_FOOD_RANGE; // intensity
                    sensory[slot + 3u] = f32(TOUCH_FOOD) / 4.0; // surface_tag
                    touch_count += 1u;
                }
            }
        }
        if touch_count >= MAX_TOUCH_CONTACTS { break; }
    }

    // Query agent_grid for nearby agents within TOUCH_AGENT_RANGE
    for (var di: i32 = -1; di <= 1; di++) {
        for (var dj: i32 = -1; dj <= 1; dj++) {
            if touch_count >= MAX_TOUCH_CONTACTS { break; }
            let ncx = self_cx + di;
            let ncz = self_cz + dj;
            if ncx < 0 || ncz < 0 { continue; }
            let uncx = u32(ncx);
            let uncz = u32(ncz);
            if uncx >= GRID_WIDTH || uncz >= GRID_WIDTH { continue; }

            let cell_idx = uncx * GRID_WIDTH + uncz;
            let cell_base = cell_idx * AGENT_GRID_CELL_STRIDE;
            let count = min(agent_grid[cell_base], AGENT_GRID_MAX_PER_CELL);

            for (var s: u32 = 0u; s < count; s++) {
                if touch_count >= MAX_TOUCH_CONTACTS { break; }
                let other = agent_grid[cell_base + 1u + s];
                if other == agent { continue; }

                let ob = other * PHYS_STRIDE;
                if agent_phys[ob + P_ALIVE] < 0.5 { continue; }

                let adx = agent_phys[ob + P_POS_X] - pos.x;
                let adz = agent_phys[ob + P_POS_Z] - pos.z;
                let dist = sqrt(adx * adx + adz * adz);

                if dist < TOUCH_AGENT_RANGE {
                    let slot = touch_base + touch_count * 4u;
                    let inv_dist = 1.0 / max(dist, 1e-6);
                    sensory[slot]      = adx * inv_dist;       // dir_x
                    sensory[slot + 1u] = adz * inv_dist;       // dir_z
                    sensory[slot + 2u] = 1.0 - dist / TOUCH_AGENT_RANGE; // intensity
                    sensory[slot + 3u] = f32(TOUCH_AGENT) / 4.0; // surface_tag
                    touch_count += 1u;
                }
            }
        }
        if touch_count >= MAX_TOUCH_CONTACTS { break; }
    }
}

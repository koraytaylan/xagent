...
/// Measure steering alignment with GPU-integrated auxiliary loss active.
/// The probe runs 100 training ticks with auxiliary loss enabled, then
/// evaluates steering alignment with pinned movement. Records alignment
/// rate and 95% CI without asserting the steering verdict (only control
/// probes are asserted). The verdict is mechanical: ACCEPT if CI lower >= 0.70,
/// REJECT if CI upper <= 0.62, otherwise inconclusive.
#[test]
fn gpu_auxiliary_steering_alignment_probe() {
    use xagent_brain::buffers::{PHYS_STRIDE, P_FOOD_COUNT};
    ...
    /// Training ticks with auxiliary loss enabled.
    const TRAIN_TICKS: usize = 100;
    /// Evaluation ticks (steering probe, pinned movement).
    const EVAL_TICKS: usize = 60;
    /// Minimum scored evaluation samples for robust measurement.
    const MIN_SCORED_SAMPLES: usize = 200;

    // Encoder separability must hold (control probe).
    let (within, between) = encoder_food_side_cosines();
    let encoder_diff = 1.0 - between;
    let encoder_within = 1.0 - within;
    let encoder_separability = if encoder_within > 1e-6 {
        encoder_diff / encoder_within
    } else {
        f32::INFINITY
    };

    // Training config: dense strides with auxiliary loss enabled.
    let train_brain = BrainConfig {
        brain_tick_stride: 1,
        vision_stride: 1,
        auxiliary_steering_loss_enabled: true,
        ..Default::default()
    };
    let mut arena = build_probe_arena(&train_brain, 17);
    arena.reset_bodies();

    // Run training ticks with auxiliary loss active.
    let mut food_consumed_train = 0.0_f32;
    for t in 0..TRAIN_TICKS {
        arena.kernel.dispatch_batch(t as u64, 1);
        let state = arena.kernel.read_full_state_blocking();
        for a in 0..PROBE_AGENT_COUNT {
            food_consumed_train += state[a * PHYS_STRIDE + P_FOOD_COUNT];
        }
    }

    // Control probe: food must be consumed during training (arena functional).
    assert!(
        food_consumed_train > 0.0,
        "no food consumed during training with auxiliary loss, arena broken"
    );

    // Control probe: encoder separability must hold (encoder not regressed).
    assert!(
        encoder_separability > 0.9,
        "encoder separability dropped to {}, possible regression",
        encoder_separability
    );

    // Switch to evaluation config (pinned movement, auxiliary loss off).
    let eval_brain = probe_brain_config();
    for a in 0..PROBE_AGENT_COUNT {
        arena
            .kernel
    ...
    // Sanity check: evaluation must have scored enough samples.
    assert!(
        scored_eval >= MIN_SCORED_SAMPLES,
        "only {} scored samples — evaluation geometry broke",
        scored_eval
    );
}

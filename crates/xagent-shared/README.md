...
| `visual_encoding_size` | 64 | **Legacy / superseded.** Replaced by the plan 0008 Hubel-Wiesel visual cortex — the structured `retina_*` / `gabor_*` / `dog_*` config plus the `coop_visual_cortex` pass. No stage in `crates/xagent-brain/src` reads this field, it is no longer editable in the UI or carried through breeding (each child resets it to the serde default), and it is retained only so older saved configs still deserialize. Resolves issue #106 by replacement. |

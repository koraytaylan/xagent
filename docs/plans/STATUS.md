...
| 0005 | GPU Occupancy & Brain-Pass Latency | 🚧 In progress | 5/6 code-complete | `0001`+`0002` wiring merged & byte-identical/green: `--bench-agent-sweep`, occupancy-sized population default (10 → **192**), `GpuKernel::has_subgroup()` + top-K-path log, and the `XAGENT_KERNEL_PASS_LIMIT` per-cooperative-pass probe. On-target tables (sweep, subgroup fact, per-pass profile, fixed-seed evolution) **pending the macOS/Metal reference GPU** (this branch ran in a no-GPU Linux container). `0003` correctly gated/open on that profile — no speculative change. | [status](0005-GPU-Occupancy-And-Brain-Latency/STATUS.md) |
